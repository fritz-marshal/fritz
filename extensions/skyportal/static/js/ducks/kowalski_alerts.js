...
export const fetchAlerts = ({ object_id, ra, dec, radius }) => {
  if (
    ...
    return API.GET(
      `/api/kowalski/alerts?objectId=${object_id}&ra=${ra}&dec=${dec}&radius=${radius}&radius_units=arcsec`,
      FETCH_ALERTS,
    );
  }
  if (object_id && Object.prototype.toString.call(object_id)) {
    return API.GET(`/api/kowalski/alerts?objectId=${object_id}`, FETCH_ALERTS);
  }
  if (ra && dec && radius) {
    return API.GET(
      `/api/kowalski/alerts?ra=${ra}&dec=${dec}&radius=${radius}&radius_units=arcsec`,
      FETCH_ALERTS,
    );
  }
  return API.GET(`/api/kowalski/alerts/${object_id}`, FETCH_ALERTS);
};

export const fetchAlertStats = () =>
  API.GET("/api/kowalski/alerts_stats", FETCH_ALERTS_STATS);
