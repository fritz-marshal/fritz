...
interface KowalskiFilterPluginsProps {
  group: any;
}

const KowalskiFilterPlugins = ({ group }: KowalskiFilterPluginsProps) => {
  ...
  const [autoFollowupRadius, setAutoFollowupRadius] = useState<any>(0.5);
  const [autoFollowupTnsAge, setAutoFollowupTnsAge] = useState<any>(null);

  const [otherVersion, setOtherVersion] = React.useState("");

  const handleSelectFilterVersionDiff = (event: any) => {
  ...
  const handleChangeAutosaveComment = async () => {
    let newAutosaveComment: any = autosaveComment;
  ...
  const handleChangeAutoFollowupComment = async () => {
    let newAutoFollowupComment: any = autoFollowupComment;
    if (newAutoFollowupComment === "") {
      newAutoFollowupComment = null;
    }
    let newAutoFollowup: any = {};
    // the the current autosave is an object and not a boolean, copy that object
  ...
  const handleChangeAutoFollowup = async (event: any) => {
    const target = event.target.checked;
    let newAutoFollowup: any = { active: target };
  ...
  const [selectedAllocationParams, setSelectedAllocationParams] =
    useState<any>(null);
  const [selectedIgnoreGroupIds, setSelectedIgnoreGroupIds] = useState<any[]>(
    [],
  );
  const [selectedTargetGroupIds, setSelectedTargetGroupIds] = useState<any[]>(
    [],
  );
    ...
    if (params.formSchema) {
      ...
      if (deleted.length > 0) {
        params.formSchema.required = (params.formSchema?.required || []).filter(
          (item: any) => !deleted.includes(item),
        );
      }
                            ...
                            <Controller
                              render={({ field: { onChange, value } }: any) => (
                                <TextareaAutosize
                                  ...
                                  style={{ width: "100%" }}
                                  ref={register("pipeline_autosave") as any}
                                  onChange={onChange}
                                  value={value}
                                />
                              )}
                  ...
                  (filter_v?.autosave?.active === true ||
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        className={(classes as any).allocationSelect}
                        multiple
                      >
                  ...
                  ((group?.group_users || []).filter(
                    ...
                    <div>
                      ...
                      >
                        ...
                        {(group?.group_users || [])
                          .filter(
                            (group_user: any) => group_user.can_save === true,
                          )
                          .map((group_user: any) => (
                            <MenuItem
                          ...
                          <Controller
                            render={({ field: { onChange, value } }: any) => (
                              <TextareaAutosize
                                ...
                                style={{ width: "100%" }}
                                ref={register("pipeline_auto_followup") as any}
                                onChange={onChange}
                                value={value}
                              />
                            )}
                        ...
                        <>
                          ...
                          >
                            ...
                            <DialogContent>
                              ...
                              <Form
                                schema={
                                  selectedAllocationParams.formSchema as any
                                }
                                validator={validator}
                                uiSchema={
                                  selectedAllocationParams.uiSchema as any
                                }
                                liveValidate
                ...
                {filter_v?.fv && filter_v?.auto_followup?.active === true && (
                  <div>
                    ...
                    <Select
                      ...
                      multiple
                    >
                      {(userAccessibleGroups || []).map((ignore_group: any) => (
                        <MenuItem
...
export default KowalskiFilterPlugins;
