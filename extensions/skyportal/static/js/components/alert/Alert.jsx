...
const getJd = (alert) => alert?.candidate?.jd;

const getRa = (alert) => alert?.candidate?.ra ?? alert?.candidate?.coord_ra;
const getDec = (alert) => alert?.candidate?.dec ?? alert?.candidate?.coord_dec;

// ── Row builder ───────────────────────────────────────────────────────────────

const buildRow = (alert, survey) => ({
...
const CatalogTable = ({ entries }) => {
  const flat = entries.map(flattenEntry);
  const allKeys = Array.from(new Set(flat.flatMap((e) => Object.keys(e))));
...
const CrossMatchesPanel = ({ cross_matches }) => {
  ...
  const catalogAccordion = (catalog, hasMatches) => {
...
CrossMatchesPanel.propTypes = {
  cross_matches: PropTypes.shape({}).isRequired,
};
...
const Alert = ({ route }) => {
  ...
  const [cutoutDataUris, setCutoutDataUris] = useState(null);
  ...
  const handlePanelXMatchChange = (panel) => (_, isExpanded) =>
  ...
  useEffect(() => {
    ...
    const fetchAlert = async () => {
      ...
      const candids = Array.from(new Set(data.data.map((a) => getCandid(a))))
        .filter((c) => c != null)
        .sort();
      const jds = Array.from(new Set(data.data.map((a) => getJd(a))))
        .filter((j) => j != null)
        .sort();
      ...
      const lastAlert = data.data.find(
        (a) => getCandid(a) === candids[candids.length - 1],
      );
  ...
  const rows = alertsForObject.map((a) => buildRow(a, survey));
